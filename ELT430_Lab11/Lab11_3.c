 ...
 *  Author:         Dinera
 *  Description:    PWM - Chopping Modul
 */
...
void fn_ePWM1_Init(void){
    ...
    // Chopping Modul
    EPwm1Regs.PCCTL.bit.CHPDUTY = 5;     // Duty 75%
    EPwm1Regs.PCCTL.bit.CHPFREQ = 4;     // Frequenz: 2,25 MHz
    EPwm1Regs.PCCTL.bit.OSHTWTH = 8;     // One Shot auf 800ns bei 90MHz stellen
    EPwm1Regs.PCCTL.bit.CHPEN = 1;       // Chopper Modul einschalten
